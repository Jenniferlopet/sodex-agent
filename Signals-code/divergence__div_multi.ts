...
export function multiDivergence(c: Candle[]): Signal {
  if (c.length < 50) return makeSignal({ reason: "Insufficient data" });
  const i = c.length - 1, a = atr(c, 14);
  const r = detectDiv(c, rsi(c.map((x) => x.close), 14), i, a, "RSI", 0.7);
  const m = detectDiv(c, macd(c.map((x) => x.close)).histogram, i, a, "MACD", 0.7);
  if (r.signal === "long" && m.signal === "long") return mk(c, i, "long", a, 0.76, "Multi-divergence: RSI + MACD both bullish");
  if (r.signal === "short" && m.signal === "short") return mk(c, i, "short", a, 0.76, "Multi-divergence: RSI + MACD both bearish");
  return makeSignal({ reason: "No multi-oscillator divergence" });
}
